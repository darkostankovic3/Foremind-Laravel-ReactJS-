...
const PortalLayout = Controller(Properties, (): JSX.Element => {
    ...
    if (!mounted) {
        return (
            <div className="flex items-center justify-center h-full w-full">
                <div>
                    <div className="w-12 h-12 rounded-full animate-spin border-y-4 border-solid border-blue-500 border-t-transparent"></div>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex min-h-full flex-col flex-grow bg-gray-100">
            ...
            <div
                className={`pb-24` + (employer?.color != null ? `` : ` bg bg-blue-500`)}
                style={{ background: employer?.color != null ? `#${employer?.color}` : `` }}
            >
                <Header />
            </div>
