...
const HomeController = Controller(Properties, (): JSX.Element => {
    ...
    const getCheckDay = (date: any) => {
        let currentDay: any;
        let checkDay: any;
        currentDay = new Date().getTime();
        checkDay = new Date(date).getTime();
        return Math.floor((Math.abs(currentDay-checkDay))/(1000*60*60*24));
    };
                ...
                <div className="flex flex-wrap flex-1 -mx-3 -my-3">
                    <div className="flex w-full xl:w-1/2 p-3">
                        {upComingAppointments && upComingAppointments.length ? (
                            ...
                            <div className="flex-1 bg-gray-100 border border-t-8 border-t-gray-400 border-gray-200 rounded-2xl flex flex-col items-center justify-center space-y-4 py-3">
                    ...
                    {check && check.created_at && (
                        <div className="flex w-full xl:w-1/2 p-3">
                            <div className="flex-1 bg-[#FBEEFF] border border-t-8 border-t-[#A21CD1] rounded-2xl flex flex-col items-center justify-center space-y-4">
                                ...
                                <div className="text-gray-400 text-xl font-bold leading-[48px]">
                                    {/* @ts-ignore */}
                                    {getCheckDay(check.created_at)} Days
                                </div>
